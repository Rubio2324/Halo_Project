...
# Importaciones para funciones de agregación y carga de relaciones (útiles si se reintroducen estadísticas o para relaciones)
...
router = APIRouter(prefix="/frontend") # Mantener este prefijo
...
def show_players(request: Request, session: Session = Depends(get_session)):
    # Cargar la relación 'team' para poder mostrar el nombre del equipo en la vista
    players = session.exec(select(Player).options(selectinload(Player.team))).all()
    teams = session.exec(select(Team)).all() # Necesario para el dropdown del formulario de búsqueda/filtro
...
def search_players(
    ...
    # Cargar la relación 'team' para poder mostrar el nombre del equipo en los resultados
